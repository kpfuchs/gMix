...
public class Tcp_Async_IO_ClientCommunicationHandler_v1 extends GeneralCommunicationHandler implements ClientCommunicationHandler, DatabaseEventListener {
		...
		public void run() {
			...
			while (true) { // handle read, write and accept events
				...
				try {
					...
					int remainingCapacity = inputOutputHandlerInternal.remainingUnprocessedRequestCapacity();
						...
						while (selectedKeys.hasNext()) {
							...
							for (int i=0; i<count; i++) 
								if (i<remainingCapacity) {
									inputOutputHandlerInternal.addUnprocessedRequest(requests.remove(0));
									remainingCapacity--;
								} else {
	...
	private class ReplyThread extends Thread {
		...
		public void run() {
			...
			while (true) { // wait for data from messageProcessor, store it in UserChannelData, add reference on UserChannelData in usersWithRepliesReady and wake up selector (which will send the data later)
				...
				Reply reply = inputOutputHandlerInternal.getProcessedReply(); // blocking method
				...
				synchronized (sync) {
					...
					UserChannelData userData = reply.getOwner().getAttachment(getThis(), UserChannelData.class);
					if (!userData.valid)
						continue;
					messageCounterReplies++;
					userData.dataToSend.add(ByteBuffer.wrap(Util.concatArrays(Util.intToByteArray(reply.getByteMessage().length), reply.getByteMessage())));
	...
	private void handleAcceptRequest() throws IOException {
		...
		if (numberOfActiveConnections < maxConnections) {
			...
			System.out.println(mix +" connection refused. too many connections ("+numberOfActiveConnections+")"); 
		}
	...
	private Request tryToReadRequest(UserChannelData userData) throws IOException {
		...
		if (userData.receivedData.hasRemaining()) {
			...
			return MixMessage.getInstanceRequest(message, userData.getOwner(), settings);
		}
	...
	private Tcp_Async_IO_ClientCommunicationHandler_v1 getThis() {
		return this;
	}
