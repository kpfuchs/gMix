...
package recodingScheme.staticFunctions;
...
public class ReplayDetectionBasic_v1 {


	private final int DB_CAPACITY;
	private final float DB_LOAD_FACTOR;
	private Hashtable<Integer, byte[]> replayDatabase;
	private static HashMap<Mix, ReplayDetectionBasic_v1> replayDetections = new HashMap<Mix, ReplayDetectionBasic_v1>();

	
	public static ReplayDetectionBasic_v1 getInstance(Mix owner) {

		ReplayDetectionBasic_v1 rd = replayDetections.get(owner);
		if (rd == null) {
			rd = new ReplayDetectionBasic_v1(owner);
	...
	private ReplayDetectionBasic_v1(Mix owner) {
