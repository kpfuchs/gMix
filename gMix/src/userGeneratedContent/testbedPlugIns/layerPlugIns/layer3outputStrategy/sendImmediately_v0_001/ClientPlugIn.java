 ...
 *******************************************************************************/
package userGeneratedContent.testbedPlugIns.layerPlugIns.layer3outputStrategy.sendImmediately_v0_001;
...
public class ClientPlugIn extends Implementation implements Layer3OutputStrategyClient {
	...
	public void connect() {
		if (anonNode.ROUTING_MODE == RoutingMode.SOURCE_ROUTING) {
			...
			this.route = dynamicRoutingPlugInClient.choseRoute();
			this.layer1.connect(this.route);
		} else {
			this.layer1.connect();
		}
	...
	public void connect(int destPseudonym) {
		if (anonNode.ROUTING_MODE == RoutingMode.SOURCE_ROUTING) {
			...
			this.route = dynamicRoutingPlugInClient.choseRoute(destPseudonym);
			this.layer1.connect(this.route);
		} else {
			this.layer1.connect();
		}
