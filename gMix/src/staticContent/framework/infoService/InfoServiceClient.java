...
public class InfoServiceClient {
	...
	public synchronized int registerAsMix(RoutingModeInfoServiceController infoServiceRoutingPlugIn) {
		connect();
		try {
			return InfoServiceServer.client_REGISTER_AS_MIX(infoServiceRoutingPlugIn, infoServiceSocket, outputStream, inputStream);
		} catch (Exception e) {
			e.printStackTrace();
			return registerAsMix(infoServiceRoutingPlugIn);
		}
	}
