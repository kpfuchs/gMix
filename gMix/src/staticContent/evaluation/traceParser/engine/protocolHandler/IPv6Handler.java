...
import staticContent.evaluation.traceParser.engine.protocolHeaderParser.IPv6Packet;
import staticContent.evaluation.traceParser.engine.protocolHeaderParser.IPpacket.IPprotocol;
...
public class IPv6Handler implements ProtocolHandler {
	...
	public String getSourceAddress(byte[] rawPacket) {
		return IPv6Packet.getSrcIPasString(rawPacket);
	}


	@Override
	public String getDestinationAddress(byte[] rawPacket) {
		return IPv6Packet.getDstIPasString(rawPacket);
	}
	...
	public int getLength(byte[] rawPacket) {
		return IPv6Packet.getPayloadLengthIncludingExtensionHeaders(rawPacket) + 40;
	}
	...
	public int getHeaderLength(byte[] rawPacket) {
		return IPv6Packet.getHeaderLengthStat(rawPacket);
	}
	...
	public byte[] getPayload(byte[] rawPacket) {
		return IPv6Packet.getPayloadStat(rawPacket);
	}


	@Override
	public Protocol getPayloadProtocol(byte[] rawPacket) {
		IPprotocol protocol = IPv6Packet.getProtocol(rawPacket);
