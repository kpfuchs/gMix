...
public class BasicDelayBox extends DelayBox {
	...
	private class SimplexDelayBox {
		...
		private double pulse; // abstand in dem pakete geschickt werden kšnnen
		...
		public int getDelay(int numberOfBytesToSend) {
			...
			// Vergangene Zeit seit letztem Aufruf berŸcksichtigen
			...
			if (!isBusy) { // Leitung frei -> neues Paket aufmachen
				...
				if (numberOfBytesToSend < freeSpaceInPacket) { // kein neues Paket nštig (Daten passen in ein Paket und fŸllen dieses nicht komplett)
				...
				if (numberOfBytesToSend < freeSpaceInPacket) { // kein neues Paket nštig (Daten passen in ein Paket und fŸllen dieses nicht komplett)
					...
					int packetsNeeded = 1 + (int)Math.floor((double)numberOfBytesToSend / (double)packetSize); // counts only full packets ("1 +" weil aktuelles nicht volles Paket auch gezŠhlt werden muss)
					freeSpaceInPacket = packetSize - (numberOfBytesToSend % packetSize);
