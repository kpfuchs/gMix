...
import simulator.misc.DiskIOHelper;
import simulator.misc.PropertyFileHelper;
...
import simulator.statistics.GeneralStatistics;
import simulator.statistics.Statistics;
import simulator.statistics.StatisticsType;
import simulator.trafficSource.TrafficSource;
...
public class Simulator {
	...
	public static void performDesiredExperiments() {
		...
		String desiredExperiments = PropertyFileHelper.getPropertyFromFile(Paths.SIM_PROPERTY_FILE_PATH, "EXPERIMENTS_TO_PERFORM");
		
		for (String desiredExperiment: desiredExperiments.split(","))
			performExperiment(Paths.SIM_EXPERIMENT_DEFINITION_FOLDER_PATH +desiredExperiment);
	...
	private static void performExperiment(String pathToPropertyFile) {
		...
		Logger.init(Paths.SIM_OUTPUT_FOLDER_PATH +ep.experimentStart +"-consoleOutput.txt");
		saveConfigToDisk(ep);
	...
	private static void saveConfigToDisk(ExperimentConfig ep) {
		...
		DiskIOHelper.writeToFile(	description + contentsOfPropertyfile, 
									Paths.SIM_OUTPUT_FOLDER_PATH +"last-config.txt");
	...
	private static void saveResultsToDisk(BigDecimal[][][][] results, ExperimentConfig ep) {
		...
		// write files
		contentForFileExplainedResultsTxt += "\n";
		DiskIOHelper.writeToFile(contentForFileExplainedResultsTxt, Paths.SIM_OUTPUT_FOLDER_PATH +"all/" + ep.experimentStart + "-explained-results.txt");
		DiskIOHelper.writeToFile(contentForFileExplainedResultsTxt, Paths.SIM_OUTPUT_FOLDER_PATH +"last-explained-results.txt");
		for (String measurand:measurands) {
			DiskIOHelper.writeToFile(contentForResultsTxts.get(measurand), Paths.SIM_OUTPUT_FOLDER_PATH +"all/" + ep.experimentStart +"-results-" +measurand +".txt");
			DiskIOHelper.writeToFile(contentForResultsTxts.get(measurand), Paths.SIM_OUTPUT_FOLDER_PATH +"last-results-" +measurand +".txt");
		}
	...
	private static void plotResults(BigDecimal[][][][] results, ExperimentConfig ep) {
		...
		for (String measurand : measurands) { // generate diagram for each measurand
			...
			String experimentDescription = "effect of " + ep.propertyToVary
					+ " on " + measurand;
			String plotScript = DiskIOHelper.getFileContent(Paths.SIM_PLOTSCRIPT_FOLDER_PATH
			...
			DiskIOHelper.writeToFile(plotScript, Paths.SIM_OUTPUT_FOLDER_PATH +"all/" + ep.experimentStart
			...
			try {
				...
				String line;
				String gnuPlotFolder = PropertyFileHelper.getPropertyFromFile(Paths.SIM_PROPERTY_FILE_PATH, "GNUPLOT_FOLDER");
				...
				DiskIOHelper.writeToFile(plotScript, Paths.SIM_OUTPUT_FOLDER_PATH +"all/" + ep.experimentStart + "-plotscript-" + measurand + ".txt");
				...
				DiskIOHelper.writeToFile(plotScript, Paths.SIM_OUTPUT_FOLDER_PATH +"last-plotscript-" + measurand + ".txt");
