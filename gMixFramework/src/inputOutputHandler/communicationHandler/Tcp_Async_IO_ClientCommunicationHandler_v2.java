...
public class Tcp_Async_IO_ClientCommunicationHandler_v2 extends GeneralCommunicationHandler implements ClientCommunicationHandler, DatabaseEventListener {
	//TODO: timeout für inaktive nutzer einführen
	...
	private class ReplyFetcher extends Thread {
		...
		public void run() {
			...
			while (true) { // wait for data from recoding scheme, store it in UserChannelData, add reference on UserChannelData in usersWithRepliesReady and wake up selector (which will send the data later)
				...
				Reply reply = inputOutputHandlerInternal.getProcessedReply(); // blocks until a reply is available
			...
			while (true) {
				...
				try {
					...
					for (int i=0; i<count; i++)
						inputOutputHandlerInternal.addUnprocessedRequest(requests.remove(0)); // might block
	...
	private Tcp_Async_IO_ClientCommunicationHandler_v2 getThis() {
		return this;
	}
