...
public class Sphinx_v1_MixHeader {
	...
	public Sphinx_v1_MixHeader(Config config, SecureRandom secureRandom) {
		this.config = config;
		this.secureRandom = secureRandom;
		byte[][] keyPair = Sphinx_v1.generateKeyPair(config);
	...
	private void computeTuples() throws Exception {
		...
		sharedSecret = Sphinx_v1.genSharedSecret(config.publicKeysOfMixes[0], blinds);

		blindingFactor = Sphinx_v1.hashB(alpha, sharedSecret);
		...
		for (int i = 1; i < config.ROUTE_LENGTH; i++) {
			...
			alpha = Sphinx_v1.genSharedSecret(alphas[i-1], blinds[i]);

			// base=public key of mix, exp= all blinds (including x)
			sharedSecret = Sphinx_v1.genSharedSecret(config.publicKeysOfMixes[i], blinds);
	...
	private void computeFillerStrings() throws Exception {
		...
		for (int i = 1; i < config.NUMBER_OF_MIXES; i++) {
			...
			byte[] paddedPhi = Util.concatArrays(_fillerStrings[i-1], Sphinx_v1.ZERO32);

			// The PRG generated with the shared secred s_(i-1)
			byte[] prg = Sphinx_v1.rho(Sphinx_v1.hashRho(_secrets[i-1], config.SECURITY_PARAMETER_SIZE), config.NUMBER_OF_MIXES, config.SECURITY_PARAMETER_SIZE);
			...
			_fillerStrings[i] = Sphinx_v1.xor(paddedPhi, prgTruncated);
		}
	}
	...
	private void computeBetasGammas(byte[] id) throws Exception {
		...
		secureRandom.nextBytes(random);	// TODO: wieso wird hier gepadded? hat header 32 byte platz fŸr dest-id?
		...
		byte[] prg = Sphinx_v1.rho(Sphinx_v1.hashRho(_secrets[config.NUMBER_OF_MIXES-1], config.SECURITY_PARAMETER_SIZE), config.NUMBER_OF_MIXES, config.SECURITY_PARAMETER_SIZE); // The PRG generated with the shared secred s_(v-1)
		...
		// beta_v-1
		byte[] beta = Util.concatArrays(Sphinx_v1.xor(paddedDestId, prgTruncated), _fillerStrings[config.NUMBER_OF_MIXES-1]);
		_betas[config.NUMBER_OF_MIXES-1] = beta;
		
		// gamma_v-1
		byte[] gamma = Sphinx_v1.mu(Sphinx_v1.hashMu(_secrets[config.NUMBER_OF_MIXES-1], config.SECURITY_PARAMETER_SIZE), beta, config.SECURITY_PARAMETER_SIZE);
		...
		for (int i=config.NUMBER_OF_MIXES-2; i>=0; i--) {
			...
			prg = Sphinx_v1.rho(Sphinx_v1.hashRho(_secrets[i], config.SECURITY_PARAMETER_SIZE), config.NUMBER_OF_MIXES, config.SECURITY_PARAMETER_SIZE);
			...
			beta = Sphinx_v1.xor(Util.concatArrays(new byte[][] {config.mixIdsSphinx[i+1], gamma, betaTruncated}), prgTruncated);
			gamma = Sphinx_v1.mu(Sphinx_v1.hashMu(_secrets[i], config.SECURITY_PARAMETER_SIZE), beta, config.SECURITY_PARAMETER_SIZE);
